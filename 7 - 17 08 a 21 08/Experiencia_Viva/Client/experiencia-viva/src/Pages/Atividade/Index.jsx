...
const columns = [
    ...
    {label: "Data do Evento", nome: "dataEvento"},
...
export default function AtividadePage(){
    ...
    async function handleSaveOrEdit(e){
        const res = await atividadeService.criar(e);
...
function AtividadeForm({open,onSave, initialData, onCancel}){
    const [formData,setFormData] = useState({nome:"",descricao:"",dataEvento:"",vagas:0});

    useEffect(() => {
        if(!initialData) return;

        const data = Date.parse(initialData?.dataEvento ?? "");
